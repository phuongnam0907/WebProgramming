    ...
    <div id="bodypage">
      <div id="main">
        ...
        <div id="products" class="row list-group"> 
        	<?php
        		$sql = "SELECT * FROM data ORDER BY rate DESC LIMIT 0,4";
			    ...
			    if (mysqli_num_rows($result) > 0) {
			      	while($row = mysqli_fetch_assoc($result)) {
			          	...
			          	$srcdow = @$row['source'];
			         	$sao = @$row['rate'];
			         	echo "<div class='item col-lg-3'><div class='thumbnail'><a href='./detail.php?value=$getid&table=data'><img class='group list-group-image' src='../data/$imgsrc' alt='$filename'></a><div class='caption'><h3 style='margin-left:15px;'>$filename</h3><div><span class='rating'>$sao/5</span></div><div class='row'><div class='col-xs-12 col-md-6'><a class='btn btn-success' href='../data/$srcdow' download='$filename' style='margin-right:23px; margin-top:-13px;'><i class='fa fa-cloud-download icon-down'></i></a></div></div></div></div></div>";
			    }
			}
        	?>
        </div>
      ...
      <div class="lady-in-on">
		...
		<div class="nbs-flexisel-container">
		  <div class="nbs-flexisel-inner">
		  	<ul id="flexiselDemo1" class="nbs-flexisel-ul" style="display: block; left: -250px;">		
		  	<?php
		  		$pp = "SELECT * FROM data WHERE type='powerpoint' ORDER BY rate DESC LIMIT 0,8";
		  		$result_pp = mysqli_query($db, $pp);
	      		...
	      		if (mysqli_num_rows($result_pp) > 0) {
	      			while($row = mysqli_fetch_assoc($result_pp)) {
				        ...
				        $srcdow = @$row['source'];
				        $sao = @$row['rate'];
				        echo "<li class='nbs-flexisel-item' style='width: 200px;'><a href='./detail.php?value=$getid&table=data'><img class='img-responsive women' src='../data/$imgsrc' alt='$filename'></a><div class='hide-in'><p style='padding-left:20px; font-size:15px;'>$filename</p><span class='rating'>$sao/5</span><span><a href='../data/$srcdow' download = '$filename' class='downtemp' style='margin-right:60px;'> Download</a></span></div></li>";
	      			}
	      		}
		  	?>
			</ul>
      ...
      <div class="lady-in-on">
		...
		<div class="nbs-flexisel-container">
		  <div class="nbs-flexisel-inner">
		  	<ul id="flexiselDemo2" class="nbs-flexisel-ul" style="display: block; left: -250px;">				
              <?php
		  		$pp = "SELECT * FROM data WHERE type='word' ORDER BY rate DESC LIMIT 0,8";
		  		$result_pp = mysqli_query($db, $pp);
      ...
      <div class="lady-in-on">
		...
		<div class="nbs-flexisel-container">
		  <div class="nbs-flexisel-inner">
		  	<ul id="flexiselDemo3" class="nbs-flexisel-ul" style="display: block; left: -250px;">				
              <?php
		  		$pp = "SELECT * FROM data WHERE type='web' ORDER BY rate DESC LIMIT 0,8";
		  		$result_pp = mysqli_query($db, $pp);
