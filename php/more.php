...
<title>Assignment - More Templates</title>
...
<style>
  ...
  @media only screen and (max-width: 770px) {.fas-start{font-size: 8px;}}
  @media only screen and (max-width: 680px) {.fas-start{font-size: 12px;}}
</style>
...
<body>
  <div id="container">
    ...
    <div id="col-12">
      	...
      	<div class="row"> 
      		<div class="col-12">
      			...
      			<div class="centerSub">
	      			<a>Maybe you need</a>
      				<br>
      				<a>Create great-looking marketing materials with customizable brochures, flyers, newsletters, business cards, and much more.</a>
      			</div>
      		</div>
      	</div>
      	<br>
      ...
      $sql = "SELECT * FROM data WHERE type='more' LIMIT 0,20";
      ...
      if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
          ...
          if ($counting == 1) {
            ...
            <a href='./detail.php?value=$getid&table=data' style='text-decoration:none;'><img src='../data/$imgsrc' alt='$filename' style='width:70%;'></a>
      ...
      ?>
      <div><hr><h1 style="text-align: center;">User Upload Files</h1></div>
      <?php
      ...
      if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
          ...
          $sao = @$row['rate'];
          if ($countt == 1) {
            echo "<div class='row'><div class='col-4'><div class='card'>
            <a href='./detail.php?value=$getid&table=tbl_uploads' style='text-decoration:none;'><img src='../data/default.jpg' alt='$filename' style='width:80%;'></a>
            <div><b>$filename</b></div>
            <div class =' box'>";
            for ($i=1; $i <=$sao ; $i++) {echo "<span class='fa fa-star fas-start checked'></span>";}
            for ($i=1; $i <=5 - $sao ; $i++) {echo "<span class='fa fa-star fas-start'></span>";}
            echo "</div><button><a href='uploads/$filename' download = '$filename' style = 'text-decoration-line: none;'>Download</a></button></div></div>";
          } else if ($countt == 2) {
            echo "<div class='col-4'><div class='card'>
            <a href='./detail.php?value=$getid&table=tbl_uploads' style='text-decoration:none;'><img src='../data/default.jpg' alt='$filename' style='width:80%;'></a>
            <div><b>$filename</b></div>
            ...
            for ($i=1; $i <=$sao ; $i++) {echo "<span class='fa fa-star fas-start checked'></span>";}
            for ($i=1; $i <=5 - $sao ; $i++) {echo "<span class='fa fa-star fas-start'></span>";}
            echo "</div><button><a href='uploads/$filename' download = '$filename' style = 'text-decoration-line: none;'>Download</a></button></div></div></div>";
          }
          }
      }
      ?>
    ...
    <div class="col-12">
      <div class="center">
        <a class="number">&laquo;</a>
        <a href="../php/more.php" class="number here">1</a>
        <a class="number">&raquo;</a>
      </div>
    </div>
      </div>
      <div style="clear:both"></div>
